...
public interface DevolucaoRepository extends JpaRepository<Devolucao, Long> {
	...
	@Query(value = "SELECT ped.numero AS numPed, c.nome_razao AS cliente, rom.numero AS numRom, CONCAT(pro.nome, ' ', cat.nome) AS produto, r.id AS idR, r.data_retirada AS dataR, r.qtd_retirada AS qtdR, r.valor_kg AS valorR, d.id AS idD, d.data_devolucao AS dataD, d.qtd_devolucao AS qtdD, d.data_pag AS dataPagD FROM prestador p INNER JOIN retirada r ON r.id_prestador = p.id INNER JOIN limpa l ON l.id = r.id_limpa INNER JOIN item_pedido ip ON ip.id = l.id_item_pedido INNER JOIN pedido ped ON ped.id = ip.id_pedido INNER JOIN cliente c ON c.id = ped.id_cliente INNER JOIN produto_romaneio pr ON pr.id = ip.id_produto_romaneio INNER JOIN produto pro ON pro.id = pr.id_produto INNER JOIN categoria cat ON cat.id = pr.id_categoria INNER JOIN romaneio rom ON rom.id = pr.id_romaneio LEFT JOIN devolucao d ON d.id_retirada = r.id WHERE p.id = :id AND (r.data_retirada BETWEEN :inicio AND :fim) ORDER BY r.data_retirada", nativeQuery = true)
