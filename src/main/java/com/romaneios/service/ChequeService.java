...
public class ChequeService {
	...
	public Cheque receber(Long id, List<RecebimentoDTO> list) {

		Cheque objSave = isExists(id);

		MovCaixa mov = new MovCaixa();

		mov.setData(LocalDate.now());
		mov.setValor(objSave.getValor());
		mov.setDescricao("Recebimento cheque n° " + objSave.getNumCheque());
		mov.setOrigem("Cheque");
		mov.setCheque(objSave);
	...
	public Cheque isExists(Long id) {
		Optional<Cheque> objSave = repository.findById(id);
